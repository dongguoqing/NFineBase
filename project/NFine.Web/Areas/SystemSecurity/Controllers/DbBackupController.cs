...
namespace NFine.Web.Areas.SystemSecurity.Controllers
{
    public class DbBackupController : ControllerBase
    {
        ...
        [HttpGet]
        [HandlerAjaxOnly]
        public ActionResult GetDatabases(string F_IP, string UserName, string Password)
        {

            var data = databaseApp.GetDataBase(F_IP,UserName,Password);
            
            List<object> list = new List<object>();
            for (int i = 0; i < data.Count; i++)
			{
                list.Add(new { id = data[i].Name, text = data[i].Name });
			} 
